...
from app.services.steelmaking import convert_weight_to_kg, load_record, replace_calculated_details
...
async def list_records(
    ...
    stmt = select(SteelmakingRecord).where(SteelmakingRecord.deleted.is_(False)).order_by(
    ...
    if q:
        like = f"%{q.strip()}%"
        stmt = stmt.where(or_(SteelmakingRecord.furnace_no.like(like), SteelmakingRecord.steel_grade.like(like)))
...
async def create_record(
    ...
    async with db.begin():
        record = SteelmakingRecord(
            **payload.model_dump(exclude={"materials", "actual_composition", "furnace_weight"}),
...
async def update_record(
    ...
    async with db.begin():
        ...
        data = payload.model_dump(exclude={"materials", "actual_composition", "furnace_weight"})
