...
async def delete_order(
    ...
    try:
        async with db.begin():
            ...
            staged = await stage_business_attachment_deletion(db, "procurement_order", [order.id])
...
async def batch_delete_orders(
    ...
    try:
        async with db.begin_nested():
            staged = await stage_business_attachment_deletion(db, "procurement_order", [order.id for order in deletable_orders])
