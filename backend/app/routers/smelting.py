...
async def list_orders(
    ...
    return PageResult(
        items=await serialize_with_creator_names(db, rows, SmeltingOrderListItem),
        total=total or 0,
        page=page,
        page_size=page_size,
    )
...
async def delete_order(
    ...
    try:
        async with db.begin():
            ...
            staged = await stage_business_attachment_deletion(db, "smelting_order", [order.id])
...
async def batch_delete_orders(
    ...
    try:
        async with db.begin_nested():
            staged = await stage_business_attachment_deletion(db, "smelting_order", [order.id for order in deletable_orders])
